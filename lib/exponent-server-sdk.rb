  ...
  module Push
    ...
    class ErrorBuilder
      ...
      def unknown_error_format(response)
        Exponent::Push::UnknownError.new("Unknown error format: #{response.respond_to?(:body) ? response.body : response.inspect}. status_message: #{response&.status_message}. debug_info: #{response&.debug_info}")      
      end
